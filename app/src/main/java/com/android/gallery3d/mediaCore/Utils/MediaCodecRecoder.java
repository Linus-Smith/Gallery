...
import com.android.gallery3d.mediaCore.recoder.InputSurface;
import com.android.gallery3d.mediaCore.recoder.OutputSurface;
...
/**
 * Created by bruce.jiang on 2016/12/12.
 */

public class MediaCodecRecoder {
    ...
    public void saveVideo(File file) throws Throwable {
        setSize(1280, 720);
        setSource(file);
//        setCopyAudio();
        setCopyVideo();
    ...
    private void awaitEncode() {
        ...
        if (mCopyVideo) {
            ...
            if(mVideoDecodedFrameCount!=mVideoEncodedFrameCount){
                if (VERBOSE) Log.d(TAG, "encoded and decoded video frame counts should match ");
            }
            if(mVideoDecodedFrameCount <= mVideoExtractedFrameCount){
                if (VERBOSE) Log.d(TAG, "decoded frame count should be less than extracted frame count ");
            }
        }
        if (mCopyAudio) {
            if(mPendingAudioDecoderOutputBufferIndices.size()==0){
                if (VERBOSE) Log.d(TAG, "no frame should be pending ");
            }
        }
...
///audio need methed start///
        ...
        encoder.setCallback(new MediaCodec.Callback() {
            ...
            public void onOutputFormatChanged(MediaCodec codec, MediaFormat format) {
                ...
                if (mOutputVideoTrack >= 0) {
                    if (VERBOSE) Log.d(TAG,"video encoder changed its output format again?");
                    return;
                }
    ...
    /**
     * Creates an extractor that reads its frames from {@link #mFile}.
     */
...
//                srcFd.getLength());
     ...
     *
     * <p>Must be called after {@link #setSize(int, int)} and {@link #setSource(File)}.
     */
    ...
    public static class SaveTask implements Runnable {
        ...
        private MediaCodecRecoder recoder;

        public SaveTask(MediaCodecRecoder recoder) {
        ...
        public void run() {
            try {
                recoder.extractDecodeEditEncodeMux();
            } catch (Throwable th) {
                mThrowable = th;
            }
        }
        ...
        public void execute()throws Throwable {
            mThread =  new Thread(this,"save vedio");
            mThread.start();
            mThread.join();
            if (mThrowable != null) {
                throw mThrowable;
